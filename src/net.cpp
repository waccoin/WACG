...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (nHost == 1)
        {
            addrConnect = CService("216.224.166.17", 80); // checkip.dyndns.org
        ...
        {
            addrConnect = CService("162.255.116.178", 80); // www.showmyip.com
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "WaccoinGold " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"AnomalyPool 1", "wacserver.ddns.net"},
    {"AnomalyPool 2", "wac.ddns.net"},
    {"Wacnode", "wacnode.ddns.net"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. WaccoinGold is probably already running."), addrBind.ToString().c_str());
        else
