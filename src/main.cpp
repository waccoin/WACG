...
uint256 hashGenesisBlock("0x0fc8f3b15cb74220aa1d552712c314df7e331b7588508b622a42ca2c3df32a1c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WaccoinGold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WaccoinGold Signed Message:\n";
...
{
    // WaccoinGold: IsDust() detection disabled, allows any valid dust to be relayed.
...
{        
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = ((pow(diff,1/1.3))/100)  * COIN;
    }
...
static const int64 nTargetTimespan = 80; // WaccoinGold: 80 sec
static const int64 nTargetSpacing = 60; // WaccoinGold: 60 sec
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // WaccoinGold: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("waccoingold-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // WaccoinGold: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // WaccoinGold: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Finished with the coin, after all the troubles we still move on";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1406942153;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 412697014;

        if (fTestNet)
        {
            block.nTime    = 1404757109;
            block.nNonce   = 412473934;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x790b97c15df2bf6b5bfbf4364cbde34d79a5c878982407f456848c21337d6aff"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // WaccoinGold: increase each by adding 2 to waccoingold's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WaccoinGoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WaccoinGoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WaccoinGoldMiner(CWallet *pwallet)
{
    printf("WaccoinGoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("waccoingold-miner");
    ...
    try { loop {
        ...
        printf("Running WaccoinGoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WaccoinGoldMiner terminated\n");
        throw;
    }
}

void GenerateWaccoinGolds(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WaccoinGoldMiner, pwallet));
}
