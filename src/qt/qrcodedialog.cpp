...
{
    QString ret = QString("waccoingold:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non WACG unit input in lnReqAmount, we generate the URI with WACG as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(WaccoinGoldUnits::format(WaccoinGoldUnits::WACG, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
